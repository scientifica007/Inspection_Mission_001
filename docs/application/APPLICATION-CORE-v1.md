...
| field observation/note recording (PRJ-03, P0; REQ-012 supporting) | `createAdHocObservation` (OBS-1) | §4.12, TRANSACTION §14, Gate 5G |
...
- `createFindingWithObservationSource(observationId, ...)` — **ensure-accounted** operation (T7) on an **EXISTING** AdHocObservation (the observation row itself is created separately by OBS-1 `createAdHocObservation`, Gate 5G — §4.12): pre-reads the AdHocObservation; if `observation.finding_id` is already set it **never** inserts another Finding and **never** overwrites/reassigns the link — the existing linked Finding is returned as the durable/idempotent result of an **identical historical retry** only, where "identical" for a request carrying a NEW-Finding draft means the durable Finding matches the requested semantic creation identity (`origin_visit_id` = the observation's Visit, `subject_id` = the observation's subject context null-safe, normalized description/`defect_type`/`defect_type_other`/location, `urgency`, `impact`, `created_by` = request actor; `created_at` and the Finding's **current** status are deliberately excluded from retry identity — a later legitimate status transition does not invalidate an identical historical retry). A durable link whose requested semantic creation target **differs** is a **conflicting retry**: typed conflict, no write, no reassignment — never silent success, and not an existing-Finding selection/re-home API (T6 remains the only correction/re-home path). Only a `finding_id IS NULL` observation receives a new Finding OPEN (origin = the observation's Visit) via a guarded link requiring `changes == 1`; it never blindly overwrites `observation.finding_id`, so a retry after "COMMIT succeeded but ACK lost" cannot create a second Finding or orphan the first one.
...
- **T10 (`transitionCorrectiveActionStatus`, Gate-5J executable contract):** allowed **CorrectiveAction** transitions are `OPEN → IN_TREATMENT`, `OPEN → RESOLVED` (legal directly), `IN_TREATMENT → RESOLVED`; there is **no VOIDED** action status and no generic `OPEN` target. The FollowUp event follows the T8 event contract (`status_target='CORRECTIVE_ACTION'`, `corrective_action_id` = the action, optional same-institution context Visit — never a PREPARATION gate, and transitions stay legal after origin-Visit finalization). A **RESOLVED** transition writes the closure fields in the **same** guarded statement: `closed_at = event_datetime` (the transition FollowUp's normalized timestamp — `closed_at` is the instant of resolution, ENTITY-CATALOG §2.12; no separate user-supplied closure time exists), mandatory meaningful `verified_by`, optional `verification_note`; IN_TREATMENT keeps every closure field NULL. T10 changes the CorrectiveAction **only** — it never alters the parent Finding (status / `status_changed_at` / sources) and never reopens the Visit. Already-at-target retries converge (`applied:false`) only on the exact durable transition-event **and** closure identity (TRANSACTION §10/§12, RECOVERY §5); a different event, mismatching closure state, duplicate matching FollowUps, or a stale retry to a surpassed target ⇒ `E_STATE_CONFLICT`.
...
### 4.12 `createAdHocObservation` (OBS-1 — Gate 5G owner-approved application contract)

**Traceability:** PROJECT — PRJ-03 (observations/notes, P0). REQ-012 is **supporting DIRECT evidence** that field observations feed the technical sheet/reporting outputs. The exact OBS-1 API and transaction contract are **owner-approved PROJECT decisions of Gate 5G** (GATE5G-DECISIONS-v1.md) — never claimed DIRECT or DERIVED from the official sources, and REQUIREMENTS-v1.md is not modified.

`createAdHocObservation({ visitId, subjectId?, text, recordedAt, recordedBy })` creates **exactly one** durable AdHocObservation identity during an open field Visit — the Observation ONLY: it never creates a Finding, never links to a Finding (`finding_id = NULL` on creation, always), never creates a Subject, never attaches Evidence, and never performs correction/reporting/UI/sync behavior. One transaction (OBS-1, TRANSACTION §14):

- **Visit gate:** the authoritative Visit read happens inside `BEGIN IMMEDIATE`; a missing Visit ⇒ `E_VISIT_NOT_FOUND`; `status <> 'PREPARATION'` or `finalized_at IS NOT NULL` ⇒ `E_VISIT_NOT_PREPARATION` — no backdated creation into a finalized Visit.
- **Optional subject:** `subjectId` NULL means a general Visit/institution-level observation. When supplied, the Subject must exist (`E_SUBJECT_NOT_FOUND`) and `subject.institution_id` must equal `visit.institution_id` (`E_CONTEXT`) — read in the same transaction; the Subject is never created or modified here, and no "active subject only" requirement is invented.
- **Content/audit:** `text` is required and stored as normalized meaningful text (trimmed; blank/whitespace-only ⇒ `E_CONFIG`). `recordedAt` is a required app-supplied timestamp (established application-core convention: ISO-8601 UTC text, never invented internally) and `recordedBy` is required meaningful normalized text — both are immutable per row (`trg_obs_bu`) and record the observation's durable entry into Visit storage.
- **Insert:** a single `INSERT INTO adhoc_observation` with `finding_id = NULL`, guarded by affected rows == 1 (B5; otherwise `ROLLBACK` + `E_STATE_CONFLICT`); the created `observation_id` comes from the adapter-compatible `lastInsertRowid`. No other row is created.
- **Idempotency — deliberately Class B:** the project has adopted no durable request/idempotency key for observation creation, and OBS-1 performs no dedupe (no UNIQUE constraint, no text/time dedupe, no hash, no sidecar table, no client request token). Two intentional invocations with identical payloads create two distinct observations. COMMIT/ACK-loss ⇒ the outcome is ambiguous to the caller and the service never auto-resubmits (RECOVERY §5).

**OBS-1 / T7 separation:** this operation always leaves `finding_id NULL` and never accepts Finding creation data. If the inspector later decides the Observation represents a Finding, the already-adopted Gate-5F T7 operation `createFindingWithObservationSource` (§4.7) handles that separately — the two operations are never combined into one transaction/API. Gate-5F semantics are unchanged.

**Deferred (documented, not implemented):** a general correction API for an informational/unlinked Observation (`correctAdHocObservation`) is NOT part of OBS-1. The data model permits observation text/subject correction while the Visit is PREPARATION, and Gate 5E supports such correction only when it rides on the specific T6 source-correction operations (§4.9). Concise boundary note: GATE5G-DECISIONS-v1.md.

### 4.13 `createCorrectiveAction` (T9 — Gate 5I executable application contract)

**Traceability:** the adopted T9 contract of TRANSACTION §10 (CorrectiveAction optional 0..*, Gate-4B aligned); entity contract `docs/data-model/ENTITY-CATALOG-v1.md` §2.12 (`action_type`/`responsible_role` value sets — DER-04 + REQ-007/011/020; `closed_at`/`verified_by` — DER-04/REQ-031; audit — base rule 18). REQUIREMENTS-v1.md is not modified.

`createCorrectiveAction({ findingId, actionType, actionTypeOther?, description, responsibleRole, responsibleRoleOther?, responsibleName?, dueDate?, createdAt, createdBy })` creates **exactly one** durable CorrectiveAction identity under **one existing Finding** — the action row ONLY. One transaction (T9, TRANSACTION §10):

- **Parent gate (in-tx):** the authoritative Finding read happens inside `BEGIN IMMEDIATE`; a missing Finding ⇒ `E_FINDING_NOT_FOUND`; parent status `OPEN` or `IN_TREATMENT` proceeds, `RESOLVED` or `VOIDED` (or any other durable status) ⇒ `E_STATE_CONFLICT` — the `trg_ca_bi` precondition is validated in-tx before the INSERT, so no raw trigger abort is ever leaked. CorrectiveActions are **optional (0..*)**: no "every Finding must have an action" rule, no "one action per Finding" rule, no dedupe by description/type/date/responsible person, no duplicate error code.
- **No Visit gate:** the corrective-action lifecycle may continue after the origin Visit has been finalized (T9 does **not** require `PREPARATION` — intentionally different from the T6 source corrections). Creating an action never reopens or modifies the Visit, never alters the Finding status / `status_changed_at` / source links, and never creates a Finding.
- **Fields:** `actionType` ∈ `MAINTENANCE_WORK`/`TASYIR_UPDATE`/`PROCUREMENT_DISTRIBUTION`/`ADMIN_ORGANIZATIONAL`/`OTHER` (else `E_CONFIG`); `actionTypeOther` required meaningful when `actionType === OTHER` (else normalized — trim, blank ⇒ NULL); `description` required meaningful normalized text; `responsibleRole` ∈ `DIRECTOR`/`CONCERNED_SERVICE`/`INSPECTOR`/`OTHER` (else `E_CONFIG`); `responsibleRoleOther` required meaningful when `responsibleRole === OTHER`; `responsibleName` optional (blank ⇒ NULL); `dueDate` optional NULL-able **TEXT preserved as supplied** — no strict format is invented (the adopted physical contract stores nullable TEXT with no mandated ISO date semantics; documented boundary, trim/blank⇒NULL normalization only); `createdAt` required app-supplied audit timestamp in the adopted strict ISO-8601 UTC form (Gate-5G convention; never invented internally); `createdBy` required meaningful normalized text.
- **Insert:** a single `INSERT INTO corrective_action` with `status = 'OPEN'` and closure fields `closed_at = NULL`, `verified_by = NULL`, `verification_note = NULL` — closure fields are **never** populated at creation (T10 owns them later) — guarded by affected rows == 1 and a present `lastInsertRowid` (B5; otherwise `ROLLBACK` + `E_STATE_CONFLICT`). No other row is created.
- **Idempotency — deliberately Class B:** no durable request/idempotency key exists for action creation (no dedupe, no sidecar table, no client token). Two intentional invocations with identical payloads create two distinct actions; after COMMIT/ACK-loss the outcome is ambiguous to the caller and the service never internally re-issues the INSERT (RECOVERY §5).
- **Creation FollowUp boundary (Gate 5I audit):** RECOVERY §2 lists a T9 "(+ optional FollowUp event)", but no authoritative document defines when it is supplied, its `status_target`/`status_after` semantics, its actor/audit payload, whether creation counts as a transition event, or its retry identity (DATA-MODEL rule 8 ties FollowUp events to status **transitions** — a row created OPEN is not a transition). T9 therefore records the action row only; a creation FollowUp is **not invented** and is reported to the owner as an open ambiguity for decision.
