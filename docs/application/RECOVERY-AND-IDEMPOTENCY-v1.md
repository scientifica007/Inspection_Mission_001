...
| `createAdHocObservation` (OBS-1, Gate 5G) | Visit gate read + optional subject validation + the single observation INSERT (`finding_id` NULL always) — nothing else is created |
| `transitionFindingStatus` / `transitionCorrectiveActionStatus` (T8/T10) | FollowUp event + status update (+ closure fields) |
| `createCorrectiveAction` (T9) | action row only — parent-Finding read/status gate in-tx + the single identity-creating INSERT (born OPEN, closure fields NULL); **no creation FollowUp is invented**: the previously listed "optional FollowUp event" has no defined authoritative payload/semantics (DATA-MODEL rule 8 ties FollowUp events to status transitions, and creation is not one) — open ambiguity, reported for owner decision |
...
- Finding/Action transitions (guarded `WHERE status = <old>`; on zero changes re-read: already at target converges **only when the durable transition-event identity matches the requested event** — for T8 that is the exact canonical FollowUp row (`finding_id` / `corrective_action_id IS NULL` / `status_target='FINDING'` / `status_after` / null-safe `visit_id` == requested contextVisitId / `event_datetime` / `actor_role` / null-safe `actor_role_other` / null-safe `actor_name` / normalized `note` / normalized `recorded_by`) plus `finding.status_changed_at == event_datetime`, with **no duplicate** matching rows; already at target with a different audit event, a mismatching `status_changed_at`, or duplicate matching rows ⇒ typed conflict — no duplicate FollowUp either way; TRANSACTION §9, Gate-5H clarification; T10 (Gate-5J) applies the same rule with `status_target='CORRECTIVE_ACTION'` / `corrective_action_id` = the action **plus** the target closure identity — IN_TREATMENT additionally requires `closed_at`/`verified_by`/`verification_note` still NULL, RESOLVED additionally requires `closed_at == event_datetime`, `verified_by` == normalized requested verifiedBy and null-safe `verification_note` == requested — and a stale retry to a target the action has already advanced beyond conflicts (TRANSACTION §10);
...
**Class B — identity-creating commands without a durable idempotency key:** `createVisit`; creating a **new** subject inside `addSubjectToScope(newSubjectData)`; `createAdHocObservation` (OBS-1, Gate 5G); `createCorrectiveAction`; similar create-new-identity operations. For class B the docs make **no exactly-once/state-idempotency claim** after "COMMIT succeeded but ACK lost". Controls: write commands are serialized; the UI prevents same-process double-tap/in-flight duplicate execution; after an uncertain crash/restart the app re-reads durable state and **surfaces the existing records for the inspector to select**, never blindly re-INSERTing. No sidecar journal and no new idempotency table in this gate.

**OBS-1 COMMIT/ACK-loss behavior (Gate 5G, explicit):** for `createAdHocObservation` the project has adopted **no** durable request/idempotency key, so after a COMMIT whose ACK is lost the outcome is ambiguous to the caller — one durable row may exist although the caller received an error. The service must **not** internally issue a second INSERT, and must **not** inspect text/time and decide that an existing row "must be" the same attempt (no auto-rerun claiming exactly-once). Recovery policy: reconstruct the durable Visit observations and **surface/reconcile the durable state to the inspector/caller**; do not auto-resubmit the create command. A deliberate later invocation — even with the exact same payload — is a new Class-B create and may legitimately produce a second observation (duplicates are not a domain error; no observation-duplicate error code exists).
