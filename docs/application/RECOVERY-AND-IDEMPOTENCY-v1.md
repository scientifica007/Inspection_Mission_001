...
| `createFindingWithObservationSource` (T7) | **ensure-accounted**: pre-read observation; if already linked → return existing Finding (no INSERT); else INSERT Finding OPEN (origin = observation's Visit) + guarded link `WHERE finding_id IS NULL` (`changes==1`; zero-row → ROLLBACK so the new Finding disappears, re-read and return the existing link) |
...
- **T7 ensure-accounted (micro-correction):** the observation is pre-read; when `observation.finding_id` is already set the operation returns the existing linked Finding without any INSERT, and the guarded link requires `finding_id IS NULL` — a retry after "COMMIT succeeded but ACK lost" never creates a second Finding and never reassigns/orphans the first one;
