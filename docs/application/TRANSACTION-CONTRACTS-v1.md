...
**T10 executable clarification (Gate 5J):** legal targets are `OPEN → IN_TREATMENT`, `OPEN → RESOLVED` (legal directly) and `IN_TREATMENT → RESOLVED`; generic target `OPEN` and every backwards/terminal move are refused (`E_STATE_CONFLICT`), and a status outside the action value set — including `VOIDED`, which does not exist for CorrectiveActions — is a malformed request (`E_CONFIG`). The FollowUp event follows the §9 T8 event contract (strict ISO-8601 UTC `event_datetime`, closed `actor_role` set, `OTHER` ⇒ meaningful `actor_role_other`, optional normalized `actor_name`, meaningful normalized `note`/`recorded_by`), and the optional context Visit follows the T8 rule (must exist — `E_VISIT_NOT_FOUND`; same institution as the parent Finding's origin Visit — `E_CONTEXT`; finalization is irrelevant — FollowUp context, never a PREPARATION gate). A **RESOLVED** transition requires a resolution payload with a mandatory meaningful `verified_by` (blank ⇒ `E_CONFIG`) and an optional `verification_note` (blank ⇒ NULL); a resolution payload on a **non-RESOLVED** target is refused (`E_CONFIG`) — closure values are never silently stored or ignored, and IN_TREATMENT keeps `closed_at`/`verified_by`/`verification_note` NULL. **`closed_at` = the transition FollowUp's normalized `event_datetime`** (ENTITY-CATALOG §2.12: `closed_at` is the instant of resolution/closure; §1.7: one transaction reuses the same timestamp for related rows; T10 is one atomic transition/closure event) — no separate user-supplied closure time exists in any authority, so none is invented. T10 changes the CorrectiveAction ONLY: it never alters the parent Finding (status / `status_changed_at` / source links) and never reopens the Visit (transitions stay legal after origin-Visit finalization).

**T10 retry identity (Gate-5J clarification of the Class-A "identical duplicates converge" rule):** "already at target" alone is NOT idempotent success. An already-at-target retry converges (`applied:false`, no second FollowUp) only when the durable transition-event identity matches the requested event exactly — the canonical FollowUp row (`finding_id` = parent Finding / `corrective_action_id` = the action / `status_target='CORRECTIVE_ACTION'` / `status_after` = requested target / null-safe `visit_id` == requested contextVisitId / `event_datetime` / `actor_role` / null-safe `actor_role_other` / null-safe `actor_name` / normalized `note` / normalized `recorded_by`) with exactly ONE matching row — **plus** the target closure identity: an IN_TREATMENT retry additionally requires `closed_at IS NULL` / `verified_by IS NULL` / `verification_note IS NULL`, and a RESOLVED retry additionally requires `closed_at == event_datetime`, `verified_by` == the normalized requested verifiedBy and `verification_note` == the normalized requested verificationNote (null-safe). Already at target with a different audit event, a mismatching closure identity, or duplicate matching FollowUp rows ⇒ `E_STATE_CONFLICT`; a stale retry to a target the action has already legitimately advanced beyond (e.g. IN_TREATMENT retry after RESOLVED) also conflicts — never backwards, never historical false-success.
...
- Finding/Action transitions (T8/T10) with the affected-row guard + state re-read — for T8, "identical" retry convergence requires the **exact durable transition-event identity** (the canonical FollowUp row of §9 plus `finding.status_changed_at == event_datetime`), never mere "already at target"; a different audit event ⇒ typed conflict (§9, Gate-5H clarification); for T10 the same rule applies with `status_target='CORRECTIVE_ACTION'` / `corrective_action_id` = the action **plus** the target closure identity — an IN_TREATMENT retry additionally requires `closed_at`/`verified_by`/`verification_note` still NULL, and a RESOLVED retry requires `closed_at == event_datetime`, `verified_by` == normalized requested verifiedBy and null-safe `verification_note` == requested; a different audit event, a mismatching closure identity, duplicate matching rows, or a stale retry to a target the action has already advanced beyond ⇒ typed conflict (§10, Gate-5J clarification);
- finalization (T11);
