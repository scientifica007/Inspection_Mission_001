...
> **الإصدار:** v1 · **المرحلة:** البوابة الثالثة — تصميم نموذج البيانات المنطقي لنسخة المفتش الميداني الأولى (MVP) — مع تصحيحَي بوابة 4A (قاعدة الانطباق) وبوابة 4B (`VOIDED`).
> **تصحيح بوابة 4B (قرار مالك مُصدَّر):** توسعة دورة حياة Finding بحالة رابعة **`VOIDED` = مُبطَل** (مسار `OPEN → VOIDED` وحيد ونهائي عند إبطال آخر مصدر بتراجع صحيح قبل إقفال زيارة الأصل — القاعدة 22)، دون توسعة حالة CorrectiveAction ودون كيان/جدول جديد.
...
- **الغرض:** بيانات مرجعية لإصدار معيّن من بند الفحص: السؤال العربي، المحور، نمط الاستجابة (قيمة مفردة/سجل)، القيم المسموحة (أطفال تابعة لنفس الإصدار)، الأولوية، التتبع، المراجع، وقواعد الملاحظة/الأدلة/توليد النقص/الانطباق.
- **قاعدة الانطباق (تصحيح بوابة 4A):** لكل إصدار **قاعدة انطباق واحدة** (`applicability_rule`) مملوكة له بالضبط — منطق «متى يُطبَّق» الخاص بذلك الإصدار (نموذج البند §3.7 ومقطع «متى يُطبَّق» لكل بند في القائمة)؛ تمثيلها الكنسي وتقييمها في `docs/checklists/APPLICABILITY-RULES-v1.md`. وهي **مُرقّمة مع التعريف وثابتة مثله** (كتغيير السؤال أو القيم أو قواعد النقص): تغيير الانطباق يستلزم إصدار تعريف جديدًا، فتبقى الاستجابات التاريخية قابلة للتفسير تحت القاعدة التي كانت سارية عند اختيار إصدار تعريفها (قاعدتا 11 و21). قاعدة البيانات تحفظ/تخزّن/تُرقّم القاعدة؛ **تقييمها** (NA/منطبق/تأكيد مفتش) مسؤولية التطبيق في بوابة 5.
- **الإصدار (versioning):** لكل تعريف `version_no` (1، 2، ...)؛ `supersedes_definition_id` (اختياري — التعريف السابق)؛ `effective_from` (تاريخ منطقي اختياري)؛ وحالة (`نشط/مستبدَل/مؤرشف`). **بمجرد استخدام إصدار ما ببيانات زيارة مثبتة، لا يُعاد كتابة سؤاله أو قيمه أو تتبعه أو قواعده (ومنها قاعدة انطباقه) أو دلالاته أبدًا**؛ أي تغيير مستقبلي يُنشئ إصدار تعريف جديدًا بنفس `item_code`، ولا تُعاد قراءة الاستجابات التاريخية بتعريف أحدث (قاعدة 11).
- **الدورة:** إصدار معتمد → نشط → مستبدَل/مؤرشف (عند إصدار إصدار جديد) — دون حذف.
- **الصفات الرئيسية:** `item_code`، `item_definition_id` (نسخة)، `version_no`، `supersedes_definition_id`، `effective_from`، المحور DOM، السؤال العربي، نمط الاستجابة، الأولوية (P0/P1/P2)، التتبع، قائمة المراجع، قواعد الملاحظة/الأدلة/النقص (نص مرجعي)، **قاعدة الانطباق `applicability_rule` (JSON كنسي v1 — إلزامية)**، الحالة.
...
- **قاعدة:** التعريف لا يُكتب أبدًا ببيانات ميدانية (قاعدة 11)؛ ولا يُنشأ كيان جديد لقوائم مجمعة (لا حاجة له — ولا `ChecklistSet`).
...
- **الدورة (مخطط الانتقالات المعتمد — قرار بوابة 4B):** `OPEN → IN_TREATMENT`، `OPEN → RESOLVED`، `IN_TREATMENT → RESOLVED`، ومسار خاص **`OPEN → VOIDED`**؛ `RESOLVED` و`VOIDED` حالتان **نهائيتان (terminal)**؛ كل انتقال للحالة يُسجَّل كحدث في FollowUp.
  - **VOIDED (مُبطَل — إضافة بوابة 4B):** نقص أُنشئ صحيحًا من مصدر ملاحظة، ثم أُبطل **آخر مصدره** بتصحيح/تراجع مشروع **وما تزال زيارة أصله في `PREPARATION`**؛ يُحتفظ بالنقص للسجل/التاريخ بدلًا من حذفه. **VOIDED ≠ RESOLVED:** الإبطال = زوال صلاحية النقص بتراجع مصدره الأخير قبل إقفال زيارة الأصل، وليس معالجة النقص والتحقق من رفعه. VOIDED يتطلب صفر مصادر وصفر إجراءات تصحيحية، ولا يعود له مصدر ولا إجراء، وهو نهائي (لا ينتقل إلى أي حالة أخرى).
  - **الإجراءات التصحيحية اختيارية (0..\*) — لا تُشترط لمجرد إقفال/رفع نقص:** إذا وُجدت إجراءات فلا يجوز أن تبقى أي منها `OPEN`/`IN_TREATMENT` عند جعل النقص `RESOLVED` (القاعدة 15)، وكل إقفال يتطلب تحقق الرفع بحدث FollowUp؛ المباشرة `OPEN → RESOLVED` مشروعة عندما تتحقق بقية شروط النزاهة المعتمدة.
- **الصفات الرئيسية:** `origin_visit_id` (الزيارة التي اكتُشف فيها النقص — سياق أصل مستقر)، الوصف (إلزامي)، نوع الخلل/النقص (اختياري — قائمة مقترحة من DER-03 + أخرى)، الموقع (اختياري)، موضوع اختياري، الاستعجال (فوري/قبل الدخول/متابعة عادية — إلزامي)، الأثر (مرتفع/متوسط/منخفض — إلزامي)، الحالة (`OPEN`/`IN_TREATMENT`/`RESOLVED`/`VOIDED` — مُبطَل إضافة بوابة 4B)، وقت/جهة التسجيل.
...
- **الهدف الانتقالي (`status_target`):** اختياري بقيم `FINDING` / `CORRECTIVE_ACTION` — عند تسجيل `status_after` يكون `status_target` **إلزاميًا**؛ وإذا كان `CORRECTIVE_ACTION` يكون `corrective_action_id` **إلزاميًا**؛ وإذا كان `FINDING` فالقيد يسجّل انتقال حالة النقص. **قيم `status_after`:** `OPEN`/`IN_TREATMENT`/`RESOLVED`/`VOIDED` — و`VOIDED` **صالحة فقط عندما `status_target = FINDING`** (قرار بوابة 4B: لا يوجد إجراء مُبطَل؛ حالة CorrectiveAction تبقى `OPEN`/`IN_TREATMENT`/`RESOLVED` فقط).
...
- **Finding:** مفتوح → قيد المعالجة → مُرفوع/مُغلق، أو **مفتوح → مُبطَل (`VOIDED`)** عند إبطال آخر مصدره بتراجع مشروع قبل إقفال زيارة أصله؛ قد يبقى **مفتوحًا بعد زيارة منشئه** ويُؤكَّد/يُتابَع في زيارات لاحقة لنفس المؤسسة؛ `VOIDED` يُحتفظ به كسجل تاريخي نهائي بلا مصادر؛ كل انتقال للحالة يُمثَّل بحدث في FollowUp ولا يمحو تاريخ المتابعة.
...
| قاعدة انطباق البند (متى يُطبَّق) | `ChecklistItemDefinition.applicability_rule` — مربوطة بـ `item_definition_id` الدقيق (قاعدة 11/21 + بوابة 4A) |
...
11. **إصدار تعريفات البنود (الفصل بين التعريف والاستجابة):** `item_code` (مثل CHK-005) معرّف تجاري مستقر عبر الإصدارات؛ `item_definition_id` معرّف داخلي ثابت **لإصدار واحد بالضبط** مع `version_no` و`supersedes_definition_id` و`effective_from`؛ **إصدار استُخدم ببيانات زيارة مثبتة لا يُعاد كتابة سؤاله أو قيمه المسموحة أو تتبعه أو قواعده (ومنها قاعدة انطباقه `applicability_rule` — بوابة 4A) أو دلالاته أبدًا**؛ أي تغيير مستقبلي — **ومنها تغيير منطق الانطباق** — يُنشئ إصدار تعريف جديدًا بنفس `item_code`؛ كل ChecklistResponse تحيل إلى الإصدار الدقيق المستخدم وقت المعاينة، و**لا تُعاد قراءة الاستجابات التاريخية بتعريف أحدث** (فتُفسَّر تحت قاعدة الانطباق السارية وقت اختيار إصدارها)؛ كل ChecklistAllowedValue تابعة لإصدار تعريف واحد بالضبط؛ الاستجابات لا تعدّل التعريف أبدًا.
...
15. **اتساق الإقفال:** لا يكون Finding «مُرفوعًا» وفي وسطه إجراء «مفتوح»/«قيد المعالجة»؛ الإقفال يتطلب تحقق الرفع (بحدث FollowUp)؛ **الإجراءات التصحيحية اختيارية (0..\*) ولا تُشترط لمجرد إقفال النقص** — المباشرة `OPEN → RESOLVED` مشروعة بشرط توفر مصدر مسجل وعدم وجود إجراءات مفتوحة/قيد المعالجة (وحالة النقص النهائية بعد حدث FollowUp للرفع)؛ انتقالات الحالة مؤرَّخة ولا تمحو تاريخ المتابعة.
...
22. **مسار إبطال النقص (`VOIDED` — قرار بوابة 4B):** يُضاف إلى دورة حياة Finding مسار وحيد للدخول إلى الحالة النهائية `VOIDED` (مُبطَل): **`OPEN → VOIDED` فقط**، والمسموح حصرًا عند تصحيح/تراجع **آخر مصدر مسجل** للنقص (استجابة غير مطابقة أو معاينة مباشرة) **وما تزال زيارة أصله (`origin_visit_id`) في `PREPARATION` وبـ `finalized_at IS NULL`**. قيود المسار: (أ) `IN_TREATMENT → VOIDED` و`RESOLVED → VOIDED` **مرفوضان**، و`VOIDED → أي حالة أخرى` مرفوض (نهائي)؛ (ب) عند لحظة الإبطال يجب أن يكون للنقص **صفر مصادر مسجلة** (لا `checklist_response.finding_id` ولا `adhoc_observation.finding_id`) و**صفر `corrective_action`**؛ (ج) كل انتقال `OPEN → VOIDED` يُمثَّل بحدث FollowUp (`status_target = FINDING` و`status_after = VOIDED`) ضمن نفس المعاملة؛ (د) لا يجوز لاحقًا ربط أي مصدر جديد أو إنشاء إجراء تصحيحي تحت نقص `VOIDED`؛ (هـ) يُحتفظ بالنقص المُبطَل في التاريخ ولا يُحذف، ولا يُعدّ عائق «يتيمًا» في إقفال الزيارة (APP: استبعاده من النقائص النشطة)؛ (و) قاعدة «مصدر واحد على الأقل قبل مغادرة OPEN» تبقى قائمة لـ `IN_TREATMENT`/`RESOLVED` ويُستثنى منها `VOIDED` وحده (فهو يمثل تحديدًا حالة ما بعد التراجع بلا مصادر) — ولا تُضعَّف قواعد المصدر التاريخية للنقائص العادية؛ (ز) `CorrectiveAction` **لا تكتسب حالة VOIDED** (حالتها تبقى `OPEN`/`IN_TREATMENT`/`RESOLVED`)، وقيمة `status_after = VOIDED` صالحة في FollowUp فقط عندما `status_target = FINDING`.
...
1. **Q1 — قواميس القيم غير المقيدة بالمصادر:** قوائم **مضبوطة لكن قابلة للتوسعة**: كل قاموس مفتوح يسمح بقيمة `أخرى (OTHER)` مع **نص حر إلزامي يصف الاختيار** عند اختيارها؛ القوائم المغلقة المقررة من المصادر (مثل الاستعجال والأثر، وحالات النقص/الإجراء) **لا تُوسَّع** إلا بقرار مشروع مُصدَّر ومُرقَّم (نسخة) — ولا تُوسَّع عشوائيًا. **(قرار مُصدَّر مطبَّق — بوابة 4B:** توسعة قائمة حالة النقص فقط بإضافة `VOIDED`؛ حالة CorrectiveAction لم تتغيّر وتظل `OPEN`/`IN_TREATMENT`/`RESOLVED`.)
...
> **حالة الملف:** نموذج منطقي للبوابة الثالثة — مع تصحيح بوابة 4A الموثق في هذه النسخة (سمة `applicability_rule` على إصدارات `ChecklistItemDefinition`، بلا كيان جديد ولا `ChecklistSet`؛ سلطته الكنسية في `docs/checklists/APPLICABILITY-RULES-v1.md`) **وتصحيح بوابة 4B الموثق أعلاه (دورة `VOIDED` للـ Finding — القاعدة 22)** — لا مخطط فيزيائي، لا SQL، لا JSON schema، لا واجهات، لا كود، لا تصدير/تقارير. لم تُعدَّل ملفات معتمدة (`README.md`، `sources/`، `docs/requirements/`، `docs/checklists/FIELD-CHECKLIST-v1.md` و`CHECKLIST-MODEL-v1.md`).
