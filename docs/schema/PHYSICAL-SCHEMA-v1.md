...
> **الإصدار:** v1 · Revision 5 (تصحيح بوابة 4B — دورة `VOIDED` (مُبطَل) على Finding فقط — بعد Revision 4/4A: جسر انطباق قوائم التحقق `applicability_rule`، وبعد Revision 3 الذي أغلق فجوات الفرض المتبقية).
> **البوابة:** الرابعة — ترجمة النموذج المنطقي المعتمد (البوابة الثالثة) إلى مخطط بيانات فيزيائي قابل للتنفيذ لنسخة المفتش الميداني الأولى (مع تصحيحَي بوابة 4A الضيق وبوابة 4B الضيق — قرار مالك مُصدَّر).
...
> **الأعداد المعتمدة الآن:** **15 جدولًا · 44 مُشغِّلًا (trigger) · منظر واحد · 24 فهرسًا صريحًا** — تأكيد بالتنفيذ الفعلي (بلا تغيير: تصحيح 4A أضاف عمودًا وقيود CHECK ووسّع مُشغِّلًا قائمًا؛ وتصحيح 4B يوسّع CHECKs قائمة ومُشغِّلات قائمة فقط — لا جدول/مُشغِّل/فهرس/منظر جديد).
> **نتيجة التحقق:** **92 اختبارًا (سلبي/إيجابي) — 0 فشل** على sqlite 3.45.1 (71 من Revision 2/3/4/4A محفوظة + 21 حالة بوابة 4B؛ انظر القسم 9؛ قابلة لإعادة التشغيل من الصفر: `python3 tests/gate4a_regression.py`).
...
## 9) التحقق التنفيذي (Regression — Revision 5 / Gate 4A + Gate 4B)
...
- **92 حالة تراكمية (سلبي/إيجابي) — 0 فشل** عبر `tests/gate4a_regression.py` (قابلة لإعادة التشغيل من الصفر): كل حالات Revision 2/3/4/4A **71** محفوظة + **21 حالة بوابة 4B**، وتشمل:
...
> **حالة الملف:** مخطط فيزيائي قابل للتنفيذ — بوابة رابعة (Revision 4: تصحيح بوابة 4A — عمود `applicability_rule` بلا أي تغيير بنيوي؛ ثم Revision 5: تصحيح بوابة 4B — دورة `VOIDED` على Finding فقط، بلا أي تغيير بنيوي). لا SQL migrations على قاعدة حية، لا واجهات، لا توليد تقارير، لا ملفات أدلة ثنائية، لا بيانات حساسة. لم تُعدَّل ملفات البوابتين الأولى والثانية المعتمدة (`README.md`، `sources/`، `docs/requirements/`، `docs/checklists/FIELD-CHECKLIST-v1.md` و`CHECKLIST-MODEL-v1.md`)؛ التغييرات حصرًا في `docs/schema/` (هذا الملف و`schema.sql` و`CONSTRAINT-MATRIX-v1.md`)، و`docs/data-model/`، و`docs/checklists/APPLICABILITY-RULES-v1.md`، و`tests/gate4a_regression.py` (المجموعة التراكمية).
