...
await ok("G6C-B-22 preflight exactly-one collision abandons candidate before stage/publish", async () => {
    ...
    await insertEvidenceRaw(w.db, "VISIT", w.ownerIds.VISIT, finalRef(50)); // historical row; physical file intentionally missing
...
await ok("G6C-B-24 final-path collision with zero DB refs preserves old object and retries", async () => {
    ...
    const svc = serviceFor(w.db, storage);
    await svc.reconcileEvidence(); // establish startup readiness before simulating post-startup residue
    storage.putFinal(finalRef(53), "old-residue", 88, HASH_B);
    const result = await svc.createEvidence(createInput("VISIT", w.ownerIds.VISIT));
...
await ok("G6C-B-26 stage failure leaves no Evidence row and cleans incoming", async () => {
    const w = await freshWorld();
    const storage = new FakeStorage([allocation(60)]);
    storage.failStage = new Error("injected stage failure");
...
await ok("G6C-B-27 hash failure is typed and leaves no Evidence row", async () => {
    const w = await freshWorld();
    const storage = new FakeStorage([allocation(61)]);
    storage.failStage = new DomainError(APP_ERR.EVIDENCE_HASH_FAILED, "injected hash failure");
...
await ok("G6C-B-28 malformed new SHA-256 is rejected before publish", async () => {
    const w = await freshWorld();
    const storage = new FakeStorage([allocation(62)]);
    storage.stageHash = `sha256:${"A".repeat(64)}`;
    await rejectsCode(APP_ERR.EVIDENCE_HASH_FAILED, () => serviceFor(w.db, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
...
await ok("G6C-B-29 invalid final published size cleans only after zero-reference proof", async () => {
    const w = await freshWorld();
    const storage = new FakeStorage([allocation(63)]);
    storage.publishedFileSizeOverride = 999;
    await rejectsCode(APP_ERR.EVIDENCE_STORAGE_WRITE_FAILED, () => serviceFor(w.db, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
...
await ok("G6C-B-30 publish failure commits no Evidence row", async () => {
    const w = await freshWorld();
    const storage = new FakeStorage([allocation(64)]);
...
await ok("G6C-B-31 transactional recheck unexpected one row => conflict/no insert/file preserved", async () => {
    const w = await freshWorld();
    const proxy = new ProxyAdapter(w.db);
    proxy.transactionalStorageRefRows = [syntheticEvidenceRow(finalRef(70), 700)];
    const storage = new FakeStorage([allocation(70)]);
    ...
    assert(proxy.evidenceInsertCount === 0, "no Evidence INSERT after drift");
    assert(storage.finals.has(finalRef(70)), "published final preserved for diagnosis");
});

await ok("G6C-B-32 transactional recheck unexpected >1 rows => conflict/no arbitrary winner", async () => {
    const w = await freshWorld();
    const proxy = new ProxyAdapter(w.db);
    proxy.transactionalStorageRefRows = [syntheticEvidenceRow(finalRef(71), 701), syntheticEvidenceRow(finalRef(71), 702)];
    const storage = new FakeStorage([allocation(71)]);
    await rejectsCode(APP_ERR.EVIDENCE_STORAGE_REF_CONFLICT, () => serviceFor(proxy, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
...
await ok("G6C-B-33 Evidence INSERT failure rolls back and removes proven orphan", async () => {
    const w = await freshWorld();
    const proxy = new ProxyAdapter(w.db);
    proxy.failEvidenceInsert = true;
    const storage = new FakeStorage([allocation(72)]);
    ...
    assert(await count(w.db, "SELECT count(*) AS c FROM evidence") === 0, "INSERT transaction rolled back");
    assert(storage.removedOrphans.includes(finalRef(72)), "final orphan removed after fresh zero proof");
});

await ok("G6C-B-34 lying changes=0 rolls back inserted row and cleans final", async () => {
    const w = await freshWorld();
    const proxy = new ProxyAdapter(w.db);
    proxy.lieEvidenceInsertChanges = 0;
    const storage = new FakeStorage([allocation(73)]);
    await rejectsCode(APP_ERR.STATE_CONFLICT, () => serviceFor(proxy, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
    assert(await count(w.db, "SELECT count(*) AS c FROM evidence") === 0, "lying cardinality row rolled back");
    assert(!storage.finals.has(finalRef(73)), "orphan cleaned");
});

await ok("G6C-B-35 missing lastInsertRowid rolls back and cleans final", async () => {
    const w = await freshWorld();
    const proxy = new ProxyAdapter(w.db);
    proxy.suppressEvidenceInsertRowid = true;
    const storage = new FakeStorage([allocation(74)]);
    await rejectsCode(APP_ERR.STATE_CONFLICT, () => serviceFor(proxy, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
...
await ok("G6C-B-36 COMMIT failure before durability => zero rows, no success, safe orphan cleanup", async () => {
    const w = await freshWorld();
    const proxy = new ProxyAdapter(w.db);
    proxy.commitMode = "FAIL_BEFORE_ONCE";
    const storage = new FakeStorage([allocation(80)]);
    await rejectsCode(APP_ERR.EVIDENCE_SQLITE_FAILED, () => serviceFor(proxy, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
    assert(await count(w.db, "SELECT count(*) AS c FROM evidence") === 0, "zero committed rows expected");
    assert(!storage.finals.has(finalRef(80)), "confirmed orphan removed");
    assert(proxy.evidenceInsertCount === 1, "must not blindly retry INSERT");
});

await ok("G6C-B-37 ACK loss after durable COMMIT + exact one match converges SUCCESS", async () => {
    const w = await freshWorld();
    const proxy = new ProxyAdapter(w.db);
    proxy.commitMode = "ACK_LOSS_AFTER_ONCE";
    const storage = new FakeStorage([allocation(81)]);
    const result = await serviceFor(proxy, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT));
    assert(result.status === "CREATED", "must converge to success");
    assert(await count(w.db, "SELECT count(*) AS c FROM evidence") === 1, "exactly one durable row");
    assert(proxy.evidenceInsertCount === 1, "no second INSERT");
});
...
await ok("G6C-B-38 ACK loss + exactly one mismatching row => STORAGE_REF_CONFLICT", async () => {
    ...
    const proxy = new ProxyAdapter(w.db);
    proxy.commitMode = "ACK_LOSS_AFTER_ONCE";
    proxy.recoveryStorageRefRows = [syntheticEvidenceRow(finalRef(82), 820, { file_name: "different.bin" })];
    const storage = new FakeStorage([allocation(82)]);
    await rejectsCode(APP_ERR.EVIDENCE_STORAGE_REF_CONFLICT, () => serviceFor(proxy, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
...
await ok("G6C-B-39 ACK loss + >1 rows => STORAGE_REF_CONFLICT/no arbitrary winner", async () => {
    ...
    const proxy = new ProxyAdapter(w.db);
    proxy.commitMode = "ACK_LOSS_AFTER_ONCE";
    proxy.recoveryStorageRefRows = [syntheticEvidenceRow(finalRef(83), 830), syntheticEvidenceRow(finalRef(83), 831)];
    const storage = new FakeStorage([allocation(83)]);
    await rejectsCode(APP_ERR.EVIDENCE_STORAGE_REF_CONFLICT, () => serviceFor(proxy, storage).createEvidence(createInput("VISIT", w.ownerIds.VISIT)));
...
await ok("G6C-B-40 uncertain-COMMIT identity ignores mutable note", async () => {
    ...
    proxy.query = async (sql: string, params: readonly SqlValue[] = []) => {
        const rows = await originalQuery(sql, params);
        if (proxy.afterAckLoss && sql.includes("WHERE storage_ref = ?") && rows.length === 1) {
            return [{ ...rows[0], note: "edited later" }];
        }
        return rows;
    };
